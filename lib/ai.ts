...
const providerOrder = (): Provider[] => {
  ...
  if (preferred !== "auto") return [preferred, ...all.filter((provider) => provider !== preferred)];
  return all;
};
...
const parseResponseText = (data: unknown): string => {
  ...
  const ollamaText = ollamaMessage && typeof ollamaMessage === "object" ? (ollamaMessage as UnknownRecord).content : record.response;
...
async function requestGroq(input: string, options: GenerateOptions) {
  ...
  if (options.webSearch && model.startsWith("groq/compound")) body.compound_custom = { tools: { enabled_tools: ["web_search", "visit_website"] } };
...
async function requestOllama(input: string, options: GenerateOptions) {
  ...
  const body: Record<string, unknown> = { model: modelFor("ollama", options), messages: [{ role: "user", content: input }], stream: false };
...
async function generateContent(input: string, options: GenerateOptions = {}) {
  ...
  // One bounded attempt per provider keeps every pipeline stage below the
  // serverless request budget. Providers are already ordered for failover.
...
export async function buildContent(topic: string, research: string, format: "short" | "long"): Promise<ResearchResult> {
  ...
  const scriptOutput = await generateContent(
    ...
    { timeoutMs: 20_000, maxTokens: format === "short" ? 500 : 1600 },
  );
  ...
  const metadataOutput = await generateContent(
    ...
    { json: true, timeoutMs: 20_000, maxTokens: 900 },
  );

  const cleaned = metadataOutput.replace(/^```json\s*/i, "").replace(/```\s*$/i, "").trim();
  const parsed: unknown = JSON.parse(cleaned);
  if (!parsed || typeof parsed !== "object") throw new Error("AI metadata response is not valid JSON.");
