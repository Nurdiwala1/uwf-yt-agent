...
const mapJob = (row: DbJob): ContentJob => ({
  id: String(row.id),
  title: String(row.title),
  topic: String(row.topic),
  format: row.format === "short" ? "short" : "long",
  state: String(row.state) as ContentJob["state"],
  scheduledFor: String(row.scheduled_for),
  createdAt: String(row.created_at),
  attempts: Number(row.attempts ?? 0),
  error: typeof row.error === "string" ? row.error : undefined,
  youtubeVideoId: typeof row.youtube_video_id === "string" ? row.youtube_video_id : undefined,
  research: typeof row.research === "string" ? row.research : undefined,
  script: typeof row.script === "string" ? row.script : undefined,
  description: typeof row.description === "string" ? row.description : undefined,
  tags: Array.isArray(row.tags) ? row.tags.filter((v): v is string => typeof v === "string") : [],
  seo: typeof row.seo === "string" ? row.seo : undefined,
  voiceId: typeof row.voice_id === "string" ? row.voice_id : undefined,
  voiceBytes: typeof row.voice_bytes === "number" ? row.voice_bytes : undefined,
  videoId: typeof row.video_id === "string" ? row.video_id : undefined,
});

const mapLog = (row: DbLog): JobLog => ({
  id: String(row.id),
...
export const db = {
  jobs: {
    ...
    insert: (job: ContentJob) => request<DbJob[]>("uwf_content_jobs", {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify({
        id: job.id, title: job.title, topic: job.topic, format: job.format, state: job.state,
        scheduled_for: job.scheduledFor, created_at: job.createdAt, attempts: job.attempts,
        error: job.error ?? null, youtube_video_id: job.youtubeVideoId ?? null, research: job.research ?? null,
        script: job.script ?? null, description: job.description ?? null, tags: job.tags ?? [], seo: job.seo ?? null,
        voice_id: job.voiceId ?? null, voice_bytes: job.voiceBytes ?? null, video_id: job.videoId ?? null,
      }),
    }).then((rows) => mapJob(rows[0])),
    update: (id: string, patch: Partial<ContentJob>) => {
      ...
      return request<DbJob[]>(`uwf_content_jobs?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify(body),
      }).then((rows) => rows[0] ? mapJob(rows[0]) : null);
    },
