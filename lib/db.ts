...
// UWF is long-form only. Legacy database rows are normalized to long format.
const mapJob = (row: DbJob): ContentJob => ({ id: String(row.id), title: String(row.title), topic: String(row.topic), format: "long", state: String(row.state) as ContentJob["state"], scheduledFor: String(row.scheduled_for), createdAt: String(row.created_at), attempts: Number(row.attempts ?? 0), error: typeof row.error === "string" ? row.error : undefined, youtubeVideoId: typeof row.youtube_video_id === "string" ? row.youtube_video_id : undefined, research: typeof row.research === "string" ? row.research : undefined, script: typeof row.script === "string" ? row.script : undefined, description: typeof row.description === "string" ? row.description : undefined, tags: Array.isArray(row.tags) ? row.tags.filter((v): v is string => typeof v === "string") : [], seo: typeof row.seo === "string" ? row.seo : undefined, voiceId: typeof row.voice_id === "string" ? row.voice_id : undefined, voiceBytes: typeof row.voice_bytes === "number" ? row.voice_bytes : undefined, videoId: typeof row.video_id === "string" ? row.video_id : undefined });
