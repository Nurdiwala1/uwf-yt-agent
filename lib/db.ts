...
const hex = (value: string) => `\\x${Buffer.from(value, "utf8").toString("hex")}`;
const unhex = (value: string) => Buffer.from(value.replace(/^\\\\x/, ""), "hex").toString("utf8");
...
export const db = {
  ...
  youtube: {
    saveRefreshToken: async (refreshToken: string, channelId: string, channelTitle?: string, thumbnail?: string) => {
      const row = { channel_id: channelId, channel_title: channelTitle ?? null, channel_thumbnail_url: thumbnail ?? null, scopes: ["youtube.upload", "youtube.readonly", "yt-analytics.readonly"], refresh_token_encrypted: hex(refreshToken), status: "connected", connected_at: new Date().toISOString(), updated_at: new Date().toISOString() };
      return request<DbJob[]>("youtube_connection", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(row) });
    },
    getRefreshToken: async () => {
      const rows = await request<DbJob[]>("youtube_connection?select=refresh_token_encrypted,status&status=eq.connected&limit=1");
      const value = rows[0]?.refresh_token_encrypted;
      if (!value) return null;
      return unhex(String(value));
    },
  },
};
