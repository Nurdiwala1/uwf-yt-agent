...
export async function runLiveStage(input: ContentJob) {
  ...
  try {
    ...
    if (current.state === "queued") {
      ...
      await store.log(current.id, "Long-form research workflow started by Run Live.");
    }
    if (current.state === "researching") {
      await store.log(current.id, "Long-form research stage running.");
    ...
    if (current.state === "scripting") {
      await store.log(current.id, "Long-form script, title, description, tags and SEO generation started.");
    ...
    if (current.state === "generating_voice") {
      await store.log(current.id, "Long-form voice generation started.");
    ...
    if (current.state === "generating_visuals") {
      if (current.videoId) {
        ...
        if (video.status === "completed") {
          ...
          await store.log(current.id, "Long-form video generation completed; publishing assembly is ready.");
          return updated;
        }
        if (video.status === "failed" || video.error) throw new Error(video.error?.message ?? "Video generation failed.");
        await store.log(current.id, `Video still ${video.status ?? "in progress"}.`);
        return current;
      }
      const prompt = `Create a premium landscape YouTube finance/crypto visual for a 5-10 minute long-form video about: ${current.topic}. Use realistic mixed visuals, clean financial graphics, charts and market imagery. No copyrighted logos. Keep visuals tightly synchronized to the narration script. The narration script is: ${current.script ?? ""}.`;
      const video = await startVideo(prompt);
      const updated = (await store.patch(current.id, { videoId: video.id }))!;
      await store.log(current.id, `Long-form video job created: ${video.id}.`);
      return updated;
    }
    if (current.state === "assembling") {
      ...
      await store.log(current.id, "Downloading completed video for YouTube upload.");
      ...
      await store.log(current.id, shouldSchedule ? `Long-form video uploaded and scheduled for ${scheduled.toISOString()}.` : `Long-form video uploaded and published on YouTube: ${videoId}.`);
      return updated;
    }
    return current;
  } catch (error) {
