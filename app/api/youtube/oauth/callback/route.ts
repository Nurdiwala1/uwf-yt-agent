...
export async function GET(request: NextRequest) {
  ...
  if (error || !code) {
    return NextResponse.redirect(new URL(`/?youtube_error=${encodeURIComponent(error ?? "No authorization code")}`, request.url));
  }
  ...
  try {
    ...
    if (!refreshToken) {
      return NextResponse.redirect(new URL("/?youtube_error=missing_refresh_token", request.url));
    }

    const response = NextResponse.redirect(new URL("/?youtube=connected", request.url));
    response.cookies.set("uwf_youtube_refresh_token", refreshToken, {
      httpOnly: true,
