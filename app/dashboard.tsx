...
function pipelineProgress(job: ContentJob | null) {
  if (!job) return 0;
  if (job.state === "published") return 100;
  const completed = pipelineStages.reduce((count, _, index) => count + (stageStatus(job, index) === "done" ? 1 : 0), 0);
...
export function Dashboard({ initialJobs, initialLogs, configured, youtubeConnected, channelStats, youtubeAnalytics }: { initialJobs: ContentJob[]; initialLogs: JobLog[]; configured: boolean; youtubeConnected: boolean; channelStats: YoutubeChannelStats | null; youtubeAnalytics: YoutubeAnalytics | null }) {
  ...
  const run = async (id: string) => { setBusy(true); const res = await fetch(`/api/jobs/${id}/run`, { method: "POST" }); const data = await res.json(); if (data.job) { setJobs((all) => all.map((j) => j.id === id ? data.job : j)); setLogs((all) => [{ id: crypto.randomUUID(), jobId: id, level: res.ok ? "info" : "error", message: res.ok ? `Live worker completed: ${data.job.state.replaceAll("_", " ")}.` : data.error, createdAt: new Date().toISOString() }, ...all]); } setBusy(false); };
  const today = jobs.filter(j => new Date(j.scheduledFor).toDateString() === new Date().toDateString());
  const activeJob = jobs.find(j => !["scheduled", "published", "failed"].includes(j.state)) ?? jobs[0] ?? null;
  ...
  return <main><aside><div className="brand"><span>UWF</span> YT Agent</div><nav><a className="active">Overview</a><a>Content pipeline</a><a>Schedule</a><a>Video library</a><a href="/settings">Settings</a></nav><div className="sidebar-foot">Operations console<br/><small>v1.0.0</small></div></aside><section className="content"><header><div><p className="eyebrow">CONTENT OPERATIONS</p><h1>Good morning, UWF.</h1><p className="muted">Your autonomous publishing desk is ready.</p></div>{youtubeConnected ? <span className="button connected-button">✓ Connected</span> : <a className="button" href="/api/youtube/oauth">{configured ? "Connect YouTube" : "Configure YouTube"}</a>}</header>
    <div className="metrics"><Metric name="Today’s jobs" value={`${today.length}/1`} hint="Publishing plan" /><Metric name="Pipeline active" value={String(jobs.filter(j => !["scheduled", "published", "failed"].includes(j.state)).length)} hint="Content in progress" /><Metric name="YouTube" value={youtubeConnected ? "Connected" : configured ? "Ready" : "Setup needed"} hint={youtubeConnected ? "Uploads authorized" : configured ? "Authorization required" : "Add OAuth secrets"} /><Metric name="API health" value="Healthy" hint="Server responding" /></div>
    <div className="grid"><section className="panel wide"><div className="panel-title"><div><p className="eyebrow">TODAY</p><h2>Publishing queue</h2></div><span className="muted">1 video / day</span></div><div className="jobs">{jobs.slice(0, 5).map(job => <article className="job" key={job.id}><div className="job-icon">{job.format === "short" ? "S" : "▶"}</div><div className="job-main"><strong>{job.title}</strong><span>{job.topic} · {job.format === "short" ? "30–60 sec Short" : "5–10 min video"}</span></div><time>{new Date(job.scheduledFor).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time><span className={`badge ${job.state}`}>{labels[job.state]}</span><button disabled={busy || ["scheduled", "published", "failed"].includes(job.state)} onClick={() => run(job.id)}>{busy ? "Working…" : "Run live"}</button></article>)}</div></section>
    ...
    <section className="panel wide"><div className="panel-title"><div><p className="eyebrow">AUTOMATION PIPELINE</p><h2>Production stages</h2></div><div style={{ minWidth: 190, textAlign: "right" }}><strong style={{ fontSize: 22 }}>{progress}%</strong><span className={`pipeline-overall ${allDone ? "all-done" : activeJob?.state === "failed" ? "has-error" : "in-progress"}`} style={{ display: "block", marginTop: 6 }}>{allDone ? "✓ All Done" : activeJob?.state === "failed" ? "✕ Error" : "Live worker execution"}</span></div></div><div style={{ height: 8, width: "100%", background: "rgba(128,128,128,.18)", borderRadius: 999, overflow: "hidden", margin: "4px 0 18px" }}><div style={{ height: "100%", width: `${progress}%`, background: "currentColor", borderRadius: 999, transition: "width .35s ease" }} /></div><div className="pipeline-checklist">{pipelineStages.map((stage, index) => { const status = stageStatus(activeJob, index); const stagePercent = status === "done" ? 10 : status === "active" ? 0 : 0; return <div key={stage} className={`pipeline-stage ${status}`}><b>{status === "pending" ? index + 1 : stageIcon(status)}</b><span>{stage}</span><em>{status === "done" ? `Complete · ${stagePercent}%` : status === "error" ? "Error" : status === "active" ? "In progress" : "Pending"}</em></div>; })}</div><p className="muted pipeline-note">{allDone ? "All production stages completed and the video has been published to YouTube." : `Pipeline completion: ${progress}% · ✓ complete, • in progress, ! error, or pending.`}</p></section>
